...
function parseHorseText(raw) {
  ...
  for (const line of lines) {
    try {
      // Format: NUM NOM JOCKEY [TRAINER] COTE VH.XX MUSIQUE
      ...
      // Extraire musique
      let musique = '';
      const musiqueMatch = line.match(/\b([1-9][p]?(?:[1-9][p]?){2,})\b/);
      if (musiqueMatch) musique = musiqueMatch[1];

      // Extraire jockey (format X.NOM ou X.X.NOM)
      let jockey = '';
      const jockeyMatch = line.match(/\b([A-Z]{1,3}\.[A-Z][A-Za-z]{2,15})\b/);
      if (jockeyMatch) jockey = jockeyMatch[1];

      // Extraire entraîneur (2ème occurrence du format jockey)
      let trainer = '';
      const jockeyMatches = [...line.matchAll(/\b([A-Z]{1,3}\.[A-Z][A-Za-z]{2,15})\b/g)];
      if (jockeyMatches.length >= 2) trainer = jockeyMatches[1][1];

      // Extraire nom (tout en MAJUSCULES entre le numéro et le jockey)
      let nom = `Cheval ${num}`;
      const nomMatch = line.match(/^\d+\s+([A-ZÉÈÀÙÂÊÎÔÛÄËÏÖÜ][A-ZÉÈÀÙÂÊÎÔÛÄËÏÖÜ\s\-']{2,30}?)(?:\s+[A-Z]{1,3}\.|$)/);
      if (nomMatch) nom = nomMatch[1].trim();

      horses.push({ num, nom, jockey, trainer, odds, vh, musique });
    } catch {}
  }
