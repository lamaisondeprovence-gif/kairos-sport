...
function AnalyzerPage() {
  ...
  const [lienEuro, setLienEuro] = useState('');
  ...
  // Analyse depuis lien EuroTiercé
  const analyzeFromUrl = async () => {
    setError('');
    if (!lienEuro || !lienEuro.includes('eurotierce.be')) {
      setError('Colle un lien EuroTiercé valide (ex: eurotierce.be/fr/course/R1/C1)');
      return;
    }
    setLoadingUrl(true);
    try {
      const res = await fetch('/api/horses-from-url', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: lienEuro })
      });
      const data = await res.json();
      if (!data.success) throw new Error(data.error || 'Impossible de lire la course');

      setScreenshotUrl(data.screenshotUrl || '');
      setCourseName(data.course || '');
      setCourseLink(data.lien || lienEuro);

      // Convertir en texte pour le parser
      const txt = data.chevaux.map(h =>
        `${h.num} ${h.nom} ${h.jockey} ${h.trainer} ${h.odds} VH.${h.vh} ${h.musique}`
      ).join('\n');
      setRawText(txt);

      // Lancer directement les 3 IA
      const withKairos = data.chevaux.map(h => {
        const ks = computeKairos(h);
        return { ...h, kairosIndex: ks.total, kairosScore: ks, verdict: getVerdict({ ...h, kairosIndex: ks.total }) };
      });

      const res3ia = await fetch('/api/horses-tri-ia', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ horses: withKairos, courseName: data.course })
      });
      const data3ia = await res3ia.json();
      if (!data3ia.success) throw new Error(data3ia.error || 'Erreur 3 IA');

      setHorses(data3ia.horses);
      setTickets(generateAllTickets(data3ia.horses, parseFloat(budget) || 100));
      setMeta3ia({ ...data3ia.meta, confianceLecture: data.confiance_lecture, distance: data.distance, terrain: data.terrain });
      setMode('3ia');
      setAnalyzed(true);
    } catch (e) {
  ...
  const reset = () => { setRawText(''); setHorses([]); setTickets([]); setAnalyzed(false); setError(''); setCourseName(''); setCourseLink(''); setMeta3ia(null); setMode('kairos'); setLienEuro(''); setScreenshotUrl(''); };
          ...
          <div style={S.scroll}>
            {!analyzed ? (
              <div style={{ paddingTop: 16 }}>
                ...
                {/* LIEN EUROTIERCÉ — ANALYSE AUTO */}
                <div style={{ ...S.cardPurple, marginBottom: 12 }}>
                  <div style={{ color: C.purple, fontSize: 9, letterSpacing: 2, marginBottom: 8, fontWeight: 700 }}>🔗 COLLE TON LIEN EUROTIERCÉ</div>
                  <input
                    value={lienEuro}
                    onChange={e => setLienEuro(e.target.value)}
                    placeholder="https://www.eurotierce.be/fr/course/R1/C1"
                    style={{ background: C.bgDeep, border: '1px solid #B66DFF44', borderRadius: 8, padding: '10px 12px', color: C.textPrimary, fontFamily: C.fontMono, fontSize: 11, width: '100%', outline: 'none', marginBottom: 10 }}
                  />
                  <button style={{ ...S.btn3ia, opacity: loadingUrl ? 0.7 : 1 }} onClick={analyzeFromUrl} disabled={loadingUrl}>
                    {loadingUrl ? '⏳ LECTURE EN COURS...' : '🚀 ANALYSER CE LIEN — 3 IA AUTO'}
                  </button>
                  {loadingUrl && (
                    <div style={{ marginTop: 10, textAlign: 'center' }}>
                      <div style={{ color: C.purple, fontSize: 10, marginBottom: 6 }}>📸 Screenshot → 👁️ Gemini lit → 🤖 3 IA analysent...</div>
                      <div style={{ display: 'flex', justifyContent: 'center', gap: 12 }}>
                        {[['📸', 'Screenshot', C.blue], ['👁️', 'Vision', C.purple], ['🤖', '3 IA', C.green]].map(([e, n, c]) => (
                          <div key={n} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 18 }}>{e}</div>
                            <div style={{ color: c, fontSize: 8, fontFamily: C.fontMono }}>{n}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{ color: C.textMuted, fontSize: 8, marginTop: 8, textAlign: 'center' }}>
                    1 lien → screenshot → Gemini lit → 3 IA analysent → tickets auto
                  </div>
                </div>

                <div style={{ color: C.textMuted, fontSize: 9, letterSpacing: 2, textAlign: 'center', marginBottom: 12 }}>— OU SAISIE MANUELLE —</div>
