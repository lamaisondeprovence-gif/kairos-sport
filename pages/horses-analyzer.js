...
function AnalyzerPage() {
  ...
  const analyze = () => {
    ...
    // ── DÉTECTION HANDICAP ──
    const vhValues = enriched.filter(h => h.vh > 0).map(h => h.vh);
    const vhMax = Math.max(...vhValues);
    const vhMin = Math.min(...vhValues);
    const vhEcart = vhMax - vhMin;
    const isHandicap = courseName.toLowerCase().includes('handicap') || vhEcart < 8;
    const topIndex = enriched[0]?.kairosIndex || 0;
    const isUncertain = isHandicap && vhEcart < 8;

    if (isUncertain) {
      enriched.forEach(h => {
        h.kairosIndex = Math.round(h.kairosIndex * 0.85);
        h.verdict = '⚠️ HANDICAP — course incertaine. Prudence maximale.';
      });
    }
              ...
              <div style={{ paddingTop: 16 }}>
                ...
                {isHandicap && (
                  <div style={{ ...S.cardRed, marginBottom: 12 }}>
                    <div style={{ color: C.red, fontWeight: 700, fontSize: 13, marginBottom: 4 }}>⚠️ COURSE HANDICAP DÉTECTÉE</div>
                    <div style={{ color: '#e8eaf0', fontSize: 11, lineHeight: 1.6 }}>
                      Les VH sont très proches entre eux. Ce type de course est difficile à prédire — les outsiders surprennent souvent. KAIROS recommande la prudence maximale ou de ne pas jouer.
                    </div>
                  </div>
                )}
