...
// Claude Vision lit la photo EuroTiercé et extrait tous les chevaux
// Puis KAIROS calcule les indices et génère le ticket
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Envoyer la photo à Claude Vision
...
- Si une valeur n'est pas visible, mets null`
    ...
    // 4. Générer l'analyse narrative avec Claude
    const narrativeResponse = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': process.env.ANTHROPIC_API_KEY,
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Tu es KAIROS, un expert hippique belge. Analyse cette course et donne un conseil de pari en 3 phrases maximum en français.

Course: ${extracted.course?.nom} - ${extracted.course?.piste} - ${extracted.course?.distance}
Top 3 KAIROS:
1. ${horses[0]?.name} @${horses[0]?.odds} - VH:${horses[0]?.vh} - Jockey:${horses[0]?.jockey} - Indice:${horses[0]?.kairosIndex}/1000
2. ${horses[1]?.name} @${horses[1]?.odds} - VH:${horses[1]?.vh} - Jockey:${horses[1]?.jockey} - Indice:${horses[1]?.kairosIndex}/1000
3. ${horses[2]?.name} @${horses[2]?.odds} - VH:${horses[2]?.vh} - Jockey:${horses[2]?.jockey} - Indice:${horses[2]?.kairosIndex}/1000

Donne ton verdict de pari en mentionnant les chevaux par leur nom. Sois direct et concis.`
        }]
      })
    });
    ...
    if (narrativeResponse.ok) {
      ...
      narrative = narrativeData.content?.[0]?.text || '';
    }
