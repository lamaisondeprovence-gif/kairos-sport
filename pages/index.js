...
export default function KairosSport() {
  ...
  const loadScanner = async (ms = 80) => {
    try {
      const res = await fetch(`/api/scanner?minScore=${ms}&t=${Date.now()}`, { cache: 'no-store' });
      const data = await res.json();
      if (data.success) { const filtered = ms > 0 ? (data.events || []).filter(e => e.kairosScore >= ms) : (data.events || []); setEvents(filtered); setStats(data.stats); setSilenceMode(filtered.length === 0); }
    } catch {}
  };
  ...
  const renderWorld = () => (
    <div style={{ paddingTop: 16 }}>
      ...
      <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 16 }}>Top 3 mondial · {stats.totalAnalyzed} événements analysés</div>
      ...
      {!worldTop3 ? <Loader text="ANALYSE EN COURS..." /> : worldTop3.map((ev, i) => (
        <div key={ev.id} style={{ ...S.card, border: `1px solid ${i === 0 ? '#FFD700' : i === 1 ? '#C0C0C0' : '#CD7F32'}44` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ flex: 1, marginRight: 12 }}>
              <div style={{ fontSize: 24, marginBottom: 4 }}>{ev.medal}</div>
              <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 4 }}>{ev.sport} {ev.competition}</div>
            ...
            <ScoreRing score={ev.kairosScore} size={64} />
          </div>
          ...
          {ev.monteCarlo && (
            <div style={{ display: 'flex', gap: 8, marginTop: 10, padding: '8px', background: 'var(--bg-deep)', borderRadius: 8 }}>
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={S.label}>Domicile</div>
                <div style={{ color: 'var(--green)', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{ev.monteCarlo.home}%</div>
              </div>
              {ev.monteCarlo.draw > 0 && <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={S.label}>Nul</div>
                <div style={{ color: 'var(--text-muted)', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{ev.monteCarlo.draw}%</div>
              </div>}
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={S.label}>Extérieur</div>
                <div style={{ color: 'var(--text-muted)', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{ev.monteCarlo.away}%</div>
              </div>
            </div>
          )}

          {/* Explication */}
          {ev.explanation && ev.explanation.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 4 }}>POURQUOI CE PARI ?</div>
              {ev.explanation.map((e, j) => <div key={j} style={{ color: 'var(--text-secondary)', fontSize: 11, padding: '2px 0' }}>{e}</div>)}
            </div>
          )}

          {/* Meilleur bookmaker */}
          {ev.bestBookmaker && (
            <a href={ev.bestBookmaker.url} target="_blank" rel="noopener noreferrer"
              ...
              🎯 JOUER SUR {ev.bestBookmaker.name} @{ev.bestBookmaker.odd}
            </a>
          )}
        </div>
      ))}
