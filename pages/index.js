...
const S = {
  ...
  cardWorld: { background: 'linear-gradient(135deg, #FF6B0015, #FFD70015)', border: '1px solid #FF6B0044', borderRadius: 14, padding: '14px 16px', marginBottom: 10 },
...
export default function KairosSport() {
  ...
  const [worldTop30, setWorldTop30] = useState(null);
  ...
  useEffect(() => { loadScanner(); loadUserStats(); loadHistory(); }, []);
    ...
    try {
      ...
      if (data.success) {
        if (data.silence) { setSilenceMode(true); setScreen(SCREENS.SILENCE); }
          ...
          if (data.psychWarnings) setPsychWarnings(data.psychWarnings);
        }
      }
    } catch {}
    setLoading(false);
  ...
  const handleWorldMode = async () => {
    setWorldLoading(true); setWorldTop30(null);
    try {
      const res = await fetch('/api/generate-ticket', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ worldMode: true, worldTop30: true, budget: parseFloat(budget) }) });
      const data = await res.json();
      if (data.success) {
        if (data.silence) { setSilenceMode(true); setScreen(SCREENS.SILENCE); }
        else {
          const top = data.top30 || data.top3 || [];
          setWorldTop30(top);
          goTo(SCREENS.WORLD);
        }
      }
  ...
  const renderPsychWarnings = () => {
    if (!psychWarnings || psychWarnings.length === 0) return null;
    return (
      <div style={{ background: '#FF4D6D15', border: '1px solid #FF4D6D44', borderRadius: 14, padding: '12px 16px', marginBottom: 10 }}>
        <div style={{ color: 'var(--red)', fontWeight: 700, fontSize: 12, marginBottom: 8, fontFamily: 'var(--font-mono)' }}>🧠 COACH PSYCHOLOGIQUE</div>
        {psychWarnings.map((w, i) => (
          <div key={i} style={{ color: 'var(--text-secondary)', fontSize: 12, padding: '4px 0' }}>{w.message}</div>
        ))}
      </div>
    );
  };

  const renderTicketCard = (t, showButtons = true) => {
    if (!t) return null;
    return (
      <div style={S.cardGreen}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ color: 'var(--green)', fontWeight: 700, fontSize: 15 }}>{t.modeLabel}</div>
          <ScoreRing score={t.globalScore} size={52} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
          {[['Sélections', t.matchCount], ['Cote totale', t.totalOdd], ['Mise', `${t.stake} €`], ['Gain potentiel', `${t.potentialGain} €`]].map(([lbl, val]) => (
        ...
        {t.valueBetsCount > 0 && (
          <div style={{ color: '#FF6B00', fontSize: 11, marginBottom: 8, fontFamily: 'var(--font-mono)' }}>🔥 {t.valueBetsCount} value bet{t.valueBetsCount > 1 ? 's' : ''} détecté{t.valueBetsCount > 1 ? 's' : ''}</div>
        )}
          ...
          <div key={i} style={{ padding: '8px 0', borderTop: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)', marginTop: 2 }}>@{m.odd} · Score {m.kairosScore}</div>
                {m.bestBookmaker && <div style={{ color: 'var(--gold)', fontSize: 10, marginTop: 2 }}>🏆 {m.bestBookmaker.name} @{m.bestBookmaker.odd}</div>}
                {m.monteCarlo && <div style={{ color: 'var(--text-muted)', fontSize: 9, fontFamily: 'var(--font-mono)', marginTop: 2 }}>🎲 Monte Carlo : {m.monteCarlo.home}% victoire</div>}
                {m.explanation && m.explanation.slice(0, 2).map((e, j) => (
                  <div key={j} style={{ color: 'var(--text-muted)', fontSize: 10, marginTop: 2 }}>{e}</div>
                ))}
              </div>
              <div style={{ marginLeft: 8 }}>
                <Badge text={m.riskLevel} color={m.riskLevel === 'Faible' ? 'var(--green)' : 'var(--gold)'} />
                {m.valueBet?.isValue && <div style={{ color: '#FF6B00', fontSize: 9, marginTop: 4, textAlign: 'right' }}>🔥 VALUE</div>}
              </div>
            </div>
          </div>
        ))}

        {t.worstMatch && <div style={{ marginTop: 8, color: 'var(--gold)', fontSize: 11 }}>⚠️ Plus risqué : {t.worstMatch}</div>}

        {showButtons && (
          <>
            {copyMsg && <div style={{ color: 'var(--green)', fontSize: 12, textAlign: 'center', margin: '8px 0', fontFamily: 'var(--font-mono)' }}>{copyMsg}</div>}
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button style={{ ...S.btnCopy, flex: 1 }} onClick={() => copyTicket(t)}>📋 COPIER</button>
              <button style={{ ...S.btnCopy, flex: 1, color: 'var(--green)', borderColor: 'var(--border-glow)' }} onClick={() => { saveToHistory(t); alert('✅ Sauvegardé !'); }}>💾 SAUVEGARDER</button>
            </div>
            <button style={{ ...S.btnPlay, marginTop: 8 }} onClick={() => window.open(t.bestBookmaker?.url || 'https://www.unibet.be', '_blank')}>🎯 JOUER MAINTENANT</button>
          </>
        )}
      </div>
    );
  ...
  const renderHome = () => (
    <div style={{ paddingTop: 20 }}>
      ...
      {/* Bouton OPPORTUNITÉS MONDIALES */}
      {worldLoading ? <Loader text="ANALYSE MONDIALE EN COURS..." /> : (
        <button style={{ ...S.btnWorld, marginBottom: 12 }} onClick={handleWorldMode}>
      ...
      {!silenceMode && events.length > 0 && (
        <div style={S.cardGreen}>
          ...
          <div style={{ color: 'var(--text-secondary)', fontSize: 13, marginBottom: 6 }}>{events[0]?.sport} {events[0]?.home} vs {events[0]?.away}</div>
          {events[0]?.trapData && <div style={{ marginBottom: 8 }}><TrapBadge trapData={events[0].trapData} /></div>}
      ...
      {silenceMode ? (
        <div style={S.cardGold}>
          ...
          <button style={{ ...S.btnSmall, borderColor: '#FFD70044', color: 'var(--gold)' }} onClick={() => { setMinScore(70); loadScanner(70); }}>Abaisser à 70</button>
        </div>
      ) : (
        <div style={{ ...S.card, marginBottom: 14 }}>
          ...
          <div style={{ color: 'var(--text-muted)', fontSize: 11, fontFamily: 'var(--font-mono)' }}>{stats.totalAnalyzed?.toLocaleString()} événements · 13 ligues</div>
        </div>
      )}
      ...
      {events.length === 0 && !silenceMode && <Loader text="CHARGEMENT..." />}
  ...
  const renderWorldMatchCard = (ev, rank) => {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ flex: 1, marginRight: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
              <span style={{ fontSize: 14 }}>{medals[rank] || `${rank+1}.`}</span>
              <span style={{ color: 'var(--text-muted)', fontSize: 9, fontFamily: 'var(--font-mono)' }}>{ev.sport} {ev.competition}</span>
            </div>
            <div style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 13 }}>{ev.home} vs {ev.away}</div>
            <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap', alignItems: 'center' }}>
              <Badge text={ev.riskLevel} color={ev.riskLevel === 'Faible' ? 'var(--green)' : 'var(--gold)'} />
              <span style={{ color: 'var(--text-muted)', fontSize: 11, fontFamily: 'var(--font-mono)' }}>@{ev.oddHome}</span>
              <ValueBadge valueBet={ev.valueBet} />
              {ev.trapData && <TrapBadge trapData={ev.trapData} />}
            </div>
          </div>
          <ScoreRing score={ev.kairosScore} size={52} />
        </div>
      </div>
    );
  };

  const renderWorldTicketBlock = (label, emoji, matches, pct, color, borderColor) => {
    if (!matches || matches.length === 0) return null;
    const budgetVal = parseFloat(budget) || 100;
    const mise = Math.round(budgetVal * pct);
    const cote = matches.reduce((a, e) => a * (e.oddHome || 1.5), 1).toFixed(2);
    const gain = Math.round(mise * parseFloat(cote));
    return (
      <div style={{ background: `${color}10`, border: `1px solid ${borderColor}`, borderRadius: 14, padding: '14px 16px', marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div>
            <span style={{ fontSize: 20 }}>{emoji}</span>
            <span style={{ color, fontWeight: 700, fontSize: 15, letterSpacing: 1, fontFamily: 'var(--font-mono)', marginLeft: 8 }}>{label}</span>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ color: 'var(--gold)', fontWeight: 700, fontSize: 15, fontFamily: 'var(--font-mono)' }}>{gain} €</div>
            <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)' }}>Mise {mise} €</div>
          </div>
        </div>
        <div style={{ background: 'var(--bg-deep)', borderRadius: 8, padding: '4px 10px', marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)' }}>COTE COMBINÉE</span>
            <span style={{ color, fontWeight: 700, fontSize: 13, fontFamily: 'var(--font-mono)' }}>×{cote}</span>
          </div>
        </div>
        {matches.map((ev, i) => renderWorldMatchCard(ev, i))}
        <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
          <button style={{ ...S.btnCopy, flex: 1, fontSize: 10 }}
            onClick={() => {
              const txt = `⚡ KAIROS ${label}\n` + matches.map(m => `${m.sport} ${m.home} vs ${m.away} @${m.oddHome}`).join('\n') + `\nCote: ×${cote} | Mise: ${mise}€ | Gain: ${gain}€`;
              navigator.clipboard.writeText(txt).then(() => setCopyMsg('✅ Copié!')).catch(() => {});
              setTimeout(() => setCopyMsg(''), 2000);
            }}>📋 COPIER</button>
          <button style={{ ...S.btnPlay, flex: 1, fontSize: 11, padding: '8px 10px' }}
            onClick={() => window.open('https://www.unibet.be', '_blank')}>🎯 JOUER</button>
        </div>
      </div>
    );
  };
  ...
  const renderWorld = () => {
    const all = worldTop30 || [];
    const ticketA = all.slice(0, 3);
    const ticketB = all.slice(3, 6);
    const ticketC = all.slice(6, 9);
    const ticketD = all.slice(9, 12);

    return (
      <div style={{ paddingTop: 16 }}>
        <button style={S.backBtn} onClick={() => goTo(SCREENS.HOME)}>← Accueil</button>
        <div style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 20, letterSpacing: 2, margin: '12px 0 4px' }}>🔥 ENTONNOIR MONDIAL</div>
        <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 4 }}>{all.length} opportunités · {stats.totalAnalyzed} événements analysés</div>
        <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 16 }}>Budget {budget}€ réparti sur {[ticketA, ticketB, ticketC, ticketD].filter(t => t.length > 0).length} tickets</div>

        {all.length === 0 ? <Loader text="ANALYSE EN COURS..." /> : (
          <>
            {/* Entonnoir visuel */}
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '12px 16px', marginBottom: 16 }}>
              <div style={{ color: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 10, letterSpacing: 2 }}>ENTONNOIR DE SÉLECTION</div>
              {[
                { label: `Top 30 scannés`, val: `${all.length} matchs`, color: 'var(--text-muted)' },
                { label: 'Ticket A · Meilleurs', val: '1-2-3', color: '#FFD700' },
                { label: 'Ticket B · Solides', val: '4-5-6', color: '#00FFB2' },
                { label: 'Ticket C · Bons', val: '7-8-9', color: '#7B61FF' },
                { label: 'Ticket D · Audacieux', val: '10-11-12', color: '#FF6B00' },
              ].map((row, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid var(--border)' }}>
                  <span style={{ color: row.color, fontSize: 12 }}>{row.label}</span>
                  <span style={{ color: row.color, fontWeight: 700, fontSize: 12, fontFamily: 'var(--font-mono)' }}>{row.val}</span>
                </div>
              ))}
            </div>
            ...
            {renderWorldTicketBlock('TICKET A · MEILLEURS', '🥇', ticketA, 0.40, '#FFD700', '#FFD70044')}
            {renderWorldTicketBlock('TICKET B · SOLIDES', '🥈', ticketB, 0.30, '#00FFB2', '#00FFB244')}
            {renderWorldTicketBlock('TICKET C · BONS', '💜', ticketC, 0.20, '#7B61FF', '#7B61FF44')}
            {ticketD.length > 0 && renderWorldTicketBlock('TICKET D · AUDACIEUX', '🔥', ticketD, 0.10, '#FF6B00', '#FF6B0044')}

            <div style={{ ...S.cardGold, marginTop: 8 }}>
              <div style={{ color: 'var(--gold)', fontWeight: 700, fontSize: 12, marginBottom: 6, fontFamily: 'var(--font-mono)' }}>💡 STRATÉGIE CONSEILLÉE</div>
              <div style={{ color: 'var(--text-secondary)', fontSize: 12, lineHeight: 1.6 }}>
                40% sur Ticket A · 30% sur Ticket B · 20% sur Ticket C · 10% sur Ticket D<br />
                Si un ticket passe, vous couvrez les autres mises.
              </div>
            </div>
          </>
        )}
  ...
  const renderScanner = () => (
    <div style={{ paddingTop: 16 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ color: 'var(--text-muted)', fontSize: 10, letterSpacing: 2, marginTop: 2, fontFamily: 'var(--font-mono)' }}>{stats.totalAnalyzed?.toLocaleString()} ÉVÉNEMENTS · 13 LIGUES</div>
      </div>
      <div style={S.card}>
      ...
      {events.length === 0 ? (
        <div style={{ ...S.cardGold, textAlign: 'center', padding: '24px 16px' }}>
          ...
          <div style={{ color: 'var(--gold)', fontWeight: 700 }}>Aucune opportunité</div>
  ...
  const renderDetail = () => {
    ...
    const bookmakers = [
      ...
      { name: 'Circus', odd: parseFloat((ev.oddHome * 1.02).toFixed(2)), url: 'https://www.circus.be' },
      { name: 'Napoleon', odd: parseFloat((ev.oddHome * 0.98).toFixed(2)), url: 'https://www.napoleon.be' },
      { name: 'Ladbrokes', odd: parseFloat((ev.oddHome * 1.01).toFixed(2)), url: 'https://www.ladbrokes.be' },
      { name: 'Betfirst', odd: parseFloat((ev.oddHome * 1.03).toFixed(2)), url: 'https://www.betfirst.be' },
    ...
    return (
      <div style={{ paddingTop: 16 }}>
        ...
        {ev.monteCarlo && (
          <div style={S.card}>
            <div style={S.label}>🎲 Simulation Monte Carlo (10 000 simulations)</div>
            <div style={{ display: 'flex', gap: 8 }}>
              <div style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'var(--bg-deep)', borderRadius: 8 }}>
                <div style={{ color: 'var(--green)', fontWeight: 700, fontSize: 20, fontFamily: 'var(--font-mono)' }}>{ev.monteCarlo.home}%</div>
                <div style={{ color: 'var(--text-muted)', fontSize: 10 }}>Domicile</div>
              </div>
              {ev.monteCarlo.draw > 0 && <div style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'var(--bg-deep)', borderRadius: 8 }}>
                <div style={{ color: 'var(--text-secondary)', fontWeight: 700, fontSize: 20, fontFamily: 'var(--font-mono)' }}>{ev.monteCarlo.draw}%</div>
                <div style={{ color: 'var(--text-muted)', fontSize: 10 }}>Nul</div>
              </div>}
              <div style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'var(--bg-deep)', borderRadius: 8 }}>
                <div style={{ color: 'var(--text-secondary)', fontWeight: 700, fontSize: 20, fontFamily: 'var(--font-mono)' }}>{ev.monteCarlo.away}%</div>
                <div style={{ color: 'var(--text-muted)', fontSize: 10 }}>Extérieur</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ ...S.card, border: '1px solid #FFD70044' }}>
          ...
          <a href={bookmakers[0].url} target="_blank" rel="noopener noreferrer"
            style={{ display: 'block', background: 'linear-gradient(135deg, #FFD700, #FF8C00)', color: '#07090f', borderRadius: 10, padding: '12px', fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 12, textTransform: 'uppercase', textAlign: 'center', textDecoration: 'none', marginTop: 10, letterSpacing: 2 }}>
  ...
  const renderGenerator = () => {
    const modes = [
      { id: 'prudent', label: '🛡️ Prudent', desc: '3 · >85' },
      { id: 'equilibre', label: '⚖️ Équilibré', desc: '5 · >80' },
      { id: 'agressif', label: '🔥 Agressif', desc: '8 · >75' },
    ];
    return (
      <div style={{ paddingTop: 16 }}>
        <button style={S.backBtn} onClick={() => goTo(SCREENS.HOME)}>← Accueil</button>
        ...
        {generatedTickets && (
          <>
            ...
            {activeTicket && renderTicketCard(activeTicket)}
          </>
        )}
    ...
    return (
      <div style={{ paddingTop: 16 }}>
        ...
        <div style={S.cardGreen}>
          ...
          {['Tickets 3 à 5 matchs', 'Cotes entre 1.20 et 1.60', 'Serie A + Premier League', 'Value bets détectés'].map(s => (
        ...
        <div style={S.cardRed}>
          ...
          {['Tickets de plus de 8 matchs', 'Paris marqués 🔴 Piège', 'Favoris cote < 1.20', 'Paris de dernière minute'].map(s => (
            <div key={s} style={{ color: 'var(--text-secondary)', fontSize: 12, padding: '5px 0', borderBottom: '1px solid var(--border)' }}>✗ {s}</div>
          ))}
        </div>
        ...
        <div style={S.cardGold}>
          ...
          <div style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.6 }}>
            {history.length === 0
              ? '« Commencez par générer votre premier ticket et marquez le résultat pour que je puisse vous coacher. »'
              : wins.length / Math.max(finished.length, 1) > 0.5
                ? '« Excellente forme ! Continuez avec des tickets prudents 3-4 matchs. »'
                : '« Revenez aux bases — tickets courts, value bets uniquement, seuil >85. »'}
          </div>
        </div>
  ...
  const renderSilence = () => (
    <div style={{ paddingTop: 60, textAlign: 'center', padding: '60px 20px 20px' }}>
      ...
      <div style={{ color: 'var(--text-muted)', fontSize: 11, margin: '12px 0', fontFamily: 'var(--font-mono)' }}>{stats.totalAnalyzed?.toLocaleString()} événements · 13 ligues analysées</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <button style={{ ...S.btnGhost, color: 'var(--gold)', borderColor: '#FFD70044' }} onClick={() => { setMinScore(70); loadScanner(70); goTo(SCREENS.SCANNER); }}>Abaisser à 70</button>
      </div>
    </div>
  );
        ...
        <div style={S.shell}>
          <div style={S.header}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              {history.length > 0 && (
                <button style={{ ...S.btnSmall, fontSize: 9, padding: '4px 8px' }} onClick={() => goTo(SCREENS.HISTORY)}>📂 {history.length}</button>
              )}
              {ticket.length > 0 && (
