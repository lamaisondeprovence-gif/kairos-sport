...
export default function HorsesPage() {
  ...
  const renderVip = () => (
    <div style={{ paddingTop: 16 }}>
      ...
      {/* Ticket du jour */}
      {ticketLoading && <Loader text="KAIROS ANALYSE LES COURSES..." />}
      ...
      {!ticketLoading && ticket && (
        <div>
          {/* Hero card ticket */}
          <div style={{ background: 'linear-gradient(135deg, #1a0d0030, #0f142280)', border: `2px solid ${ticket.kairosIndex >= 900 ? C.green : ticket.kairosIndex >= 850 ? C.gold : C.orange}`, borderRadius: 18, padding: '20px 16px', marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              <div>
                <div style={{ color: C.textMuted, fontSize: 9, letterSpacing: 2, fontFamily: C.fontMono }}>TICKET DU JOUR</div>
                <div style={{ color: C.textPrimary, fontWeight: 700, fontSize: 16, marginTop: 2 }}>{ticket.type}</div>
                <div style={{ color: C.textMuted, fontSize: 10, fontFamily: C.fontMono, marginTop: 2 }}>
                  {ticket.race.country} {ticket.race.track} · {ticket.race.time}
                  {ticket.race.countdown?.label && (
                    <span style={{ color: ticket.race.countdown.urgent ? C.red : C.textMuted, marginLeft: 6 }}>
                      ⏱️ {ticket.race.countdown.label}
                    </span>
                  )}
                </div>
              </div>
              <KairosHorseBadge index={ticket.kairosIndex} size="large" />
            </div>

            {/* Chevaux sélectionnés */}
            {ticket.horses.map((h, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: `1px solid ${C.border}` }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <span style={{ color: C.gold, fontWeight: 900, fontSize: 16, fontFamily: C.fontMono, width: 22 }}>{h.num}</span>
                  <div>
                    <div style={{ color: C.textPrimary, fontWeight: 700, fontSize: 15 }}>{h.name}</div>
                    <div style={{ color: C.textMuted, fontSize: 9, fontFamily: C.fontMono }}>{h.jockey} · VH {h.vh || 0} · {h.forme}</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ color: C.orange, fontWeight: 900, fontSize: 16, fontFamily: C.fontMono }}>@{h.odds}</div>
            ...
            {/* Pièges détectés */}
            {ticket.traps?.length > 0 && (
              <div style={{ marginTop: 10, padding: '8px 10px', background: '#FF4D6D10', borderRadius: 8, border: '1px solid #FF4D6D30' }}>
                <div style={{ color: C.red, fontSize: 10, fontWeight: 700, marginBottom: 4, fontFamily: C.fontMono }}>⚠️ PIÈGES DÉTECTÉS</div>
                {ticket.traps.map((t, i) => <div key={i} style={{ color: C.textMuted, fontSize: 10 }}>✗ {t}</div>)}
              </div>
            )}

            {/* Mise & Gain */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14 }}>
              <div style={{ background: C.bgDeep, borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
                <div style={{ color: C.orange, fontWeight: 900, fontSize: 18, fontFamily: C.fontMono }}>{ticket.mise}€</div>
                <div style={{ color: C.textMuted, fontSize: 9, letterSpacing: 1 }}>Mise Kelly</div>
              </div>
              <div style={{ background: C.bgDeep, borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
                <div style={{ color: C.green, fontWeight: 900, fontSize: 18, fontFamily: C.fontMono }}>+{ticket.gainPotentiel}€</div>
                <div style={{ color: C.textMuted, fontSize: 9, letterSpacing: 1 }}>Gain potentiel</div>
              </div>
            </div>

            {/* Narration IA */}
            <div style={{ marginTop: 12, padding: '10px', background: '#FFD70010', borderRadius: 8, border: '1px solid #FFD70020' }}>
              <div style={{ color: C.gold, fontSize: 10, fontWeight: 700, marginBottom: 4 }}>💡 ANALYSE KAIROS</div>
              <div style={{ color: C.textSecondary, fontSize: 12, lineHeight: 1.6 }}>{ticket.narrative}</div>
            </div>
          </div>

          {/* BOUTON JOUER */}
          <button style={S.btnPlay} onClick={() => setShowConfirm(true)}>
            🎯 JOUER SUR EUROTIERCÉ →
          </button>
          <div style={{ color: C.textMuted, fontSize: 9, textAlign: 'center', marginTop: 6, fontFamily: C.fontMono }}>
            {ticket.deepLinkLabel} · Course {ticket.race.time}
            {ticket.race.countdown?.urgent && <span style={{ color: C.red }}> · ⚠️ {ticket.race.countdown?.label}</span>}
          </div>
          ...
          {showConfirm && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: '#000000cc', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
              <div style={{ background: C.bgCard, border: `2px solid ${C.orange}`, borderRadius: 18, padding: 24, width: '100%', maxWidth: 380 }}>
                <div style={{ color: C.orange, fontWeight: 900, fontSize: 18, letterSpacing: 2, marginBottom: 16, textAlign: 'center' }}>⚡ CONFIRMER</div>
                <div style={{ color: C.textPrimary, fontSize: 15, fontWeight: 700, marginBottom: 4, textAlign: 'center' }}>{ticket.horses[0]?.name}</div>
                <div style={{ color: C.textMuted, fontSize: 11, textAlign: 'center', marginBottom: 16, fontFamily: C.fontMono }}>{ticket.type} · @{ticket.horses[0]?.odds} · {ticket.race.track}</div>
                {/* Slider mise */}
                <div style={{ marginBottom: 16 }}>
                  <div style={{ color: C.textMuted, fontSize: 9, letterSpacing: 2, marginBottom: 6 }}>CHOISIR LA MISE</div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {[2, 5, 10, 20, 50, ticket.mise].map(m => (
                      <button key={m} style={{ background: (misePerso || ticket.mise) == m ? C.orange : C.bgDeep, color: (misePerso || ticket.mise) == m ? '#07090f' : C.textPrimary, border: `1px solid ${C.border}`, borderRadius: 8, padding: '6px 12px', fontFamily: C.fontMono, fontSize: 12, cursor: 'pointer', fontWeight: 700 }}
                        onClick={() => setMisePerso(String(m))}>
                        {m}€
                      </button>
                    ))}
                  </div>
                  <div style={{ color: C.green, fontSize: 13, marginTop: 8, fontFamily: C.fontMono, fontWeight: 700 }}>
                    Gain potentiel : +{((parseFloat(misePerso || ticket.mise) * ticket.horses[0]?.odds) * 0.9).toFixed(2)}€
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                  <button style={{ ...S.btnGhost, flex: 1 }} onClick={() => setShowConfirm(false)}>Annuler</button>
                  <button style={{ ...S.btnPlay, flex: 2, padding: '14px', fontSize: 13 }} onClick={() => {
                    setShowConfirm(false);
                    saveToHistory(ticket, 'pending');
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button style={{ ...S.btnGhost, flex: 1, fontSize: 10 }} onClick={generateTicket}>🔄 Regénérer</button>
            <button style={{ ...S.btnGhost, flex: 1, fontSize: 10 }} onClick={() => setScreen('scanner')}>🔍 Courses</button>
            <button style={{ background: '#25D36620', color: '#25D366', border: '1px solid #25D36644', borderRadius: 12, padding: '10px 8px', fontFamily: C.fontMono, fontSize: 10, cursor: 'pointer', flex: 1 }}
              onClick={() => {
                const msg = `🐎 KAIROS HORSES\n${ticket.type} · ${ticket.horses[0]?.name} @${ticket.horses[0]?.odds}\nIndice: ${ticket.kairosIndex}/1000\nMise: ${ticket.mise}€ → +${ticket.gainPotentiel}€\n${ticket.deepLink}`;
