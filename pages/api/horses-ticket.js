...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!bestHorse || bestIndex < 750) {
      return res.status(200).json({
        success: true,
        silence: true,
        message: `Indice max aujourd'hui : ${bestIndex}/1000. Pas d'opportunité premium. Attendez demain.`,
      });
    }
    ...
    if (bestIndex >= 900) {
      // SIMPLE GAGNANT — mise max
      ticketType = 'Simple Gagnant';
      ticketHorses = [bestHorse];
      mise = Math.min(budgetNum * 0.08, budgetNum); // 8% bankroll Kelly
      gainPotentiel = mise * bestHorse.odds * 0.9; // -10% commission
      description = `KAIROS a une confiance exceptionnelle (${bestIndex}/1000). Simple gagnant recommandé.`;
    } else if (bestIndex >= 850) {
      // COUPLÉ avec 2ème meilleur
      if (top2.length >= 2 && top2[1].kairosIndex >= 750) {
        ticketType = 'Couplé Ordre';
        ticketHorses = top2;
        mise = Math.min(budgetNum * 0.06, budgetNum);
        gainPotentiel = mise * top2[0].odds * top2[1].odds * 0.9 * 0.4;
        description = `Deux chevaux forts détectés. Couplé dans l'ordre recommandé.`;
      } else {
        ticketType = 'Simple Gagnant';
        ticketHorses = [bestHorse];
        mise = Math.min(budgetNum * 0.06, budgetNum);
      ...
      if (top3.length >= 3) {
        ...
        description = `Indice correct mais prudence. Trio recommandé pour sécuriser.`;
      } else {
        ticketType = 'Simple Placé';
        ticketHorses = [bestHorse];
        mise = Math.min(budgetNum * 0.04, budgetNum);
        gainPotentiel = mise * Math.min(bestHorse.odds * 0.4, 3.0) * 0.9;
        description = `Pari placé — risque limité.`;
      }
    } else {
      // SILENCE
      return res.status(200).json({
        success: true,
        silence: true,
        message: `Indice maximum ${bestIndex}/1000 — insuffisant. KAIROS recommande d'attendre.`,
      });
    }
