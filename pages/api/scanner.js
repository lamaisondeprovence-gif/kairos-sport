...
const ALL_LEAGUES = {
  ...
  // ── EUROPE COUPES ──
  2: { name: 'UEFA Champions League', country: 'EU', tier: 1 },
  3: { name: 'UEFA Europa League', country: 'EU', tier: 1 },
  848: { name: 'UEFA Conference League', country: 'EU', tier: 2 },
  // ── AMERIQUES ──
  253: { name: 'Major League Soccer', country: 'US', tier: 2 },
  71: { name: 'Serie A Brésil', country: 'BR', tier: 2 },
...
async function buildFootballEvents() {
  ...
  // Max 50 matchs pour respecter les 7500 req/jour
  ...
  try {
    ...
    if (cached && cached.length > 0) {
      ...
      if (allEvents.length === 0) {
        return res.status(200).json({
          ...
          stats: { totalAnalyzed: 0, premiumCount: 0, ignoredCount: 0 },
          events: [],
          message: 'Aucun match disponible dans les ligues analysees pour le moment.',
        });
      }
    ...
    if (premium.length === 0) {
      return res.status(200).json({
        ...
        events: [],
        message: `${allEvents.length} matchs analyses — aucun ne depasse le seuil de ${minScore}. Conservez votre capital.`,
      });
    }
