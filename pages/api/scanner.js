...
async function buildFootballEvents() {
  ...
  // Chercher dans les deux saisons possibles
  ...
  // D'abord chercher par DATE (tous les matchs du jour sans filtre de ligue)
  try {
    ...
    for (const f of fixtures) {
      if (!seenIds.has(f.fixture.id)) {
        ...
        const leagueInfo = ALL_LEAGUES[leagueId] || { name: f.league.name, country: f.league.country?.substring(0, 3) || '🌍', tier: 3 };
        allFixtures.push({ ...f, leagueInfo, leagueId });
      }
    }
  } catch {}

  // Ensuite chercher par ligue pour les compétitions importantes
  for (const [leagueId, leagueInfo] of Object.entries(ALL_LEAGUES)) {
    for (const season of seasons) {
      try {
        const fixtures = await fetchAPI(`/fixtures?date=${today}&league=${leagueId}&season=${season}&status=NS`);
        for (const f of fixtures) {
          if (!seenIds.has(f.fixture.id)) {
            seenIds.add(f.fixture.id);
  ...
  // Trier : ligues connues d'abord, puis par tier
  allFixtures.sort((a, b) => {
    const tierA = a.leagueInfo.tier || 3;
    const tierB = b.leagueInfo.tier || 3;
    return tierA - tierB;
  });
  ...
  for (const fixture of limitedFixtures) {
    try {
      ...
      const isTopLeague = (fixture.leagueInfo.tier || 3) === 1;
      ...
      analyzedEvents.push({
        ...
        leagueTier: fixture.leagueInfo.tier || 3,
    ...
    if (cached && cached.length > 0) {
      ...
      if (allEvents.length === 0) {
        return res.status(200).json({
          ...
          stats: { totalAnalyzed: 0, premiumCount: 0, ignoredCount: 0 },
          events: [],
          message: 'Aucun match disponible pour le moment. Revenez plus tard.',
        });
      }
