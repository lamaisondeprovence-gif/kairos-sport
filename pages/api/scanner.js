...
const TOP_LEAGUES = {
  ...
  88: { name: 'Eredivisie', country: 'NL', tier: 2 },
  94: { name: 'Liga Portugal', country: 'PT', tier: 2 },
  144: { name: 'Pro League', country: 'BE', tier: 2 },
    ...
    if (cached && cached.length > 0) {
      ...
      if (allEvents.length === 0) {
        return res.status(200).json({
          ...
          stats: { totalAnalyzed: 0, premiumCount: 0, ignoredCount: 0 },
          events: [],
          message: 'Aucun match disponible dans les ligues analysées pour le moment.',
        });
      }
    ...
    if (premium.length === 0) {
      return res.status(200).json({
        ...
        stats: {
          ...
          premiumCount: 0,
          ignoredCount: allEvents.length,
        },
        events: [],
        message: `${allEvents.length} matchs analysés — aucun ne dépasse le seuil de ${minScore}. Conservez votre capital.`,
      });
    }
